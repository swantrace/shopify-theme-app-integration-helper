...
function getOptionsByProductId(productId, shopifyURL) {
...
function hasOption(productId, shopifyURL) {
...
function removeClonedButtonAndResumeOriginalButton(
  form,
...
function deleteOptionProductByProductId(productId) {
  if (
    window.BOLD &&
    window.BOLD.options &&
...
function unloadOptionsOfForm(
  ...
  const optionWidgets = Array.from(form.getElementsByClassName("bold_options"));
  ...
  removeClonedButtonAndResumeOriginalButton(form, atcButtonSelector);
  deleteOptionProductByProductId(productId);
  return true;
}

function reloadOptionsOfForm(
  form,
  productId,
  referenceNode = form.firstChild,
  moveIfExist = false,
...
function updateAffirmPriceOnPOTotalChanged(
  atcForm,
  productJSON,
  ...
  if (
    ...
    window.BOLD.common.eventEmitter.on("BOLD_OPTIONS_total_changed", function({
      data
    }) {
    ...
    window.BOLD.common.eventEmitter.on("BOLD_COMMON_variant_changed", function(
      data
    ) {
...
function showLoadingSignAsSoonAsClonedButtonIsClicked(
  showLoadingSignFunc,
...
function fixScrollIssueOnNarrative(defaultClassNames) {
  onATCFormResize(form => {
    if (form.closest(`.${defaultClassNames.join(".")}`)) {
      const wrapper = form.closest(`.${defaultClassNames.join(".")}`);
      wrapper.className = defaultClassNames.join(" ");
    }
